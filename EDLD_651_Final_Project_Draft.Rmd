...
grad <- import(here("data", "2005-2010__Graduation_Outcomes_-__By_Borough.csv")) %>% 
  clean_names() %>% 
  as_tibble() # condensed this into a single pipeline


summary(grad$cohort) # needs to be cleaned in new df, change Aug 2006 to 2006

clean_grad <- grad
clean_grad$cohort <-  as.numeric(sub("Aug 2006", "2006", grad$cohort)) # is there perhaps a better name for this cohort given that there is already a separate 2006 cohort?
...
```{r pivots}
messy_grad <- clean_grad %>% # I like how you use pivot_wider and pivot_longer simply to demonstrate your proficiency. Noice.
...
dropout_percent <- filter(clean_grad_2, demographic == "English Language Learners" |
...
<!-- This really helped me understand the data! I never would have considered blah blah blah until I read your discussion. -->
