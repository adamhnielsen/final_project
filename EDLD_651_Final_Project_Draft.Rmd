...
<!-- Everything is looking great so far! Just a friendly reminder that you can customize your code chunks based on what you want to show in your document.  -->
```{r import_and_clean}
...
# Do we want to use recode() or rename()? Also, does it make more sense to leave all of the columns until we get to the 'select' r-chunk? -Adam
...
<!-- This was helpful to see as we also had tidy data. We were struggling with untidying and re-tidying so it was nice to see someone do so successfully. -->
...
demographic_bar %>% 
  ...
  geom_bar(position = "stack", stat = "identity") + # do we want stack or dodge for position? -Adam
  theme(axis.text.x = element_text(angle = 60)) +
  facet_wrap(~cohort) +
  scale_fill_viridis_d()
...
```
<!-- Great way to visualize the data! I learned from the way you created the "English Language Learners variable. One suggestion may be to have "cohort" as a factor so it colors as more of a categorical variable rather than a scale?  -->
