...
###MK### You can also pipe the clean_names and as_tibble functions with import if you want to save some space. It'd be something like this:
###MK### grad <- import(here("data", "2005-2010__Graduation_Outcomes_-__By_Borough.csv")) %>% clean_names() %>% as_tibble()

summary(grad$cohort) # needs to be cleaned in new df, change Aug 2006 to 2006

clean_grad <- grad
clean_grad$cohort <-  as.numeric(sub("Aug 2006", "2006", grad$cohort)) ###MK###This is super cool!
...
clean_grad %>% 
  ...
  mutate(`English Language Learner Status` = ###MK### Just as a suggestion, a shorter name with no spaces may be useful since it seems that you will use this variable more than once.
           factor(demographic, 
  ...
  geom_jitter(aes(color = cohort)) + facet_wrap(~borough) + ###MK### Very cool!
...
###MK### The plot looks cool. One thing that may make the plot easier on the eye could be to have the legend at the bottom of the plot (i.e., theme(legend.position = "bottom", legend.direction = "horizontal")) or to make the figure wider via `r graph_results, fig.width=8.5`.
```
